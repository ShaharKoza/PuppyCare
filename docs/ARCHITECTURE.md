...
Implements a `FoodAssistantQuerying` protocol with a local rule-based lookup table covering common foods and their dog-safety status. The protocol design means the entire service can be swapped for a remote inference backend without changing any view code.
